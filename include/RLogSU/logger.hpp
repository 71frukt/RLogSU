...
#define     RLSU_ERROR(   std_format_, ...) do {RLSU::Log::UnitLogger.Log(__FILE__, __LINE__, __func__, RLSU::Log::Logger::ERROR  , std_format_, ##__VA_ARGS__);} while(0)
    ...
    #define RLSU_ASSERT(condition, ...) do {                                                                                              \
        if (!(condition)) {                                                                                                                            \
            RLSU::Log::UnitLogger.Log(__FILE__, __LINE__, __func__, RLSU::Log::Logger::ASSERT , #condition " failed. " __VA_OPT__(__VA_ARGS__));    \
            abort();                                                                                                                                   \
        }                                                                                                                                              \
    } while(0)
